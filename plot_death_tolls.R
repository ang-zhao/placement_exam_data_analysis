...
tab20 = tables[[3]] #20th century table
tab21 = tables[[4]] #21st century table

## Please ignore this, I scraped the wrong table at first.
...
convert_dt <- function(dt) {
  ...
  if(str_detect(dt, "-") || str_detect(dt, "–")) { #check if death toll is a range, i.e. contains "-"
...
disasters$`Death toll` = mapply(convert_dt, disasters$`Death toll`)

##################################
###### Plotting ###########
##################################

disasters %>% ggplot(aes(x = Year, y = `Death toll`, color = Type)) +
  geom_point() +
